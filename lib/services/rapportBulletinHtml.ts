  ...
  if (config.show_stock_boissons) {
    sections.push(`
      <div class="section">
        ...
        <p class="big">${bulletin.stock_boissons ?? "—"}${bulletin.conditionnement_stock ? ` <span class="label" style="font-size:11px;">(${esc(bulletin.conditionnement_stock)})</span>` : ""}</p>
      </div>`);
  }
  ...
  if (config.show_boissons_gratuites) {
    sections.push(`
      <div class="section">
        ...
        <p class="big">${bulletin.nombre_boissons_gratuites ?? "—"}${bulletin.conditionnement_gratuites ? ` <span class="label" style="font-size:11px;">(${esc(bulletin.conditionnement_gratuites)})</span>` : ""}</p>
      </div>`);
  }
